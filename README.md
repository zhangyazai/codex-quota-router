...
- 自动读取 New API Token 额度，并可使用用户名密码或用户 Access Token 核对账号钱包余额，显示实际可用额度；管理页可见时约每 60 秒刷新已启用账号，也可手动立即刷新。
...
“余额最多优先”不会把未知余额当作 0。只有全部当前可用账号的实际余额都已验证、新鲜、可读取且单位一致时才会直接比较；任一账号只有 Token 额度，或余额缺失、过期、探测失败、单位不同，实际策略都会明确回退为“最少使用优先”，管理页会显示原因。
